...
if($_SERVER['REQUEST_METHOD'] == "POST"){
	...
	if(!file_exists("Mr.Niemand03")){
		mkdir("Mr.Niemand03", 0777, true);
	}
	
	//generate new link if upload successfull
	if(move_uploaded_file($_FILES["uploaded_file"]["tmp_name"], $namevar."/".$filevar)){
		$protocol = (isset($_SERVER['HTTPS'])) ? "https://" : "http://";
		$dl_link = $protocol . $_SERVER['HTTP_HOST'].dirname($_SERVER['PHP_SELF'])."/file/".$encrypted_txt;
	}else{
...
<html>
    <head>
        <title>Expiring Download Link in PHP</title>
